...
{ 0, 1, 8, 16, 9, 2, 3, 10, 17, 24, 32, 25, 18, 11, 4, 5, 12, 19, 26, 33, 40,
		48, 41, 34, 27, 20, 13, 6, 7, 14, 21, 28, 35, 42, 49, 56, 57, 50, 43,
...
static void fillInBuf(void)
{
	...
	PJPG_MAX_IN_BUF_SIZE - gInBufOfs, &gInBufLeft, g_pCallback_data);
	if (status)
	{
...
{
	return ((x < getExtendTest(s)) ?
			((int16) x + getExtendOffset(s)) : (int16) x);
}
...
static uint8 initFrame(void)
{
	...
	gMaxMCUSPerRow = (gImageXSize + (gMaxMCUXSize - 1))
			>> ((gMaxMCUXSize == 8) ? 3 : 4);
	gMaxMCUSPerCol = (gImageYSize + (gMaxMCUYSize - 1))
			>> ((gMaxMCUYSize == 8) ? 3 : 4);
...
const uint8 gWinogradQuant[] =
		{ 128, 178, 178, 167, 246, 167, 151, 232, 232, 151, 128, 209, 219, 209,
				128, 101, 178, 197, 197, 178, 101, 69, 139, 167, 177, 167, 139,
				69, 35, 96, 131, 151, 151, 131, 96, 35, 49, 91, 118, 128, 118,
				91, 49, 46, 81, 101, 101, 81, 46, 42, 69, 79, 69, 42, 35, 54,
